...
int main(){
    ...
    for (int i = 0; i < 24; i++){
        Mat src = imread("../"+std::__cxx11::to_string(i)+"_orig.jpg");
        Mat gray_img;
    ...
    calibrateCamera( obj_points, img_points, img_size, camera_matrix, dist_coeffs, rvecs, tvecs );


    Mat org, rst;
    for (int i = 0; i < 24; i++){
        org = imread("../"+std::__cxx11::to_string(i)+"_orig.jpg");
        undistort(org, rst,camera_matrix,dist_coeffs);
        imwrite("../result/"+std::__cxx11::to_string(i)+"_dst.jpg",rst);
    }
    return 0;
